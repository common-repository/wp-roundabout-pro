...
wp_register_script( 'shapejs', plugins_url( 'js/shape/jquery.roundabout-shapes.min.js', __FILE__ ));
...
mysql_query("CREATE TABLE IF NOT EXISTS `".$wpdb->prefix."rbt_sliders` (
...
mysql_query("CREATE TABLE IF NOT EXISTS `".$wpdb->prefix."rbt_slides` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
...
$data = mysql_fetch_assoc(mysql_query("SELECT * FROM `".$wpdb->prefix."rbt_sliders` WHERE `id`='$slider_id'"));

$res = mysql_query("SELECT * FROM `".$wpdb->prefix."rbt_slides` WHERE `sliderid`='$slider_id'");
...
echo'<a href="http://phploaded.com" class="boozv" target="_blank">plugin by phploaded.com</a>';
